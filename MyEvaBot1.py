...
def get_move(square):
    ...
    if len(enemy_dir)>0:
        for i in (NORTH, EAST, SOUTH, WEST):
            ...
            n_enemy = len(enemy_group) # the overkill is in the diagonal direction
            n_self = len(self_group)+1# total number of self (3-n_enemy+1) 1 is from the square move
            ...
            strength_my_total = sum(s.strength for s in self_group) +square.strength # possible total loss, reinforcement strength in the cell
            if n_enemy > 0 :
                strength_max = max(en.strength for en in enemy_group) # possible move of the cell of the enemy
            else:
                strength_max = 0
        ...
        td = sum(min([neighbors[k].strength, square.strength]) for k in dir1 if neighbors[k].owner not in (0,myID))
